...
function buildRaceStartDate(raceDate?: string, raceTime?: string): Date | null {
...
export default function PredictScreen() {
  ...
  const { nextRace, drivers, getTeamById } = useSeriesData();
  ...
  useEffect(() => {
    ...
    if (!nextRace.hasSprint && activeTab === 'sprint') {
      setActiveTab('race');
    }
  }, [nextRace, activeTab]);
  ...
  useEffect(() => {
    ...
    if (lastSyncedRaceId !== nextRace.id) {
      setSelectedDrivers([]);
      setSprintTop8([]);
      setFastestLap(null);
      setDnf(null);
    ...
    (driverId: string) => {
      ...
      setSelectedDrivers((prev) => {
        ...
        if (cleaned.length >= raceTopN) {
          ...
          Alert.alert('Race Grid Full', `Remove a ${seriesLabels.competitorLower} from your race grid first.`);
          return cleaned;
        }
  ...
  const removeRaceDriver = useCallback(
    (driverId: string) => {
      ...
      setSelectedDrivers((prev) =>
        cleanPickList(prev, raceTopN, validDriverIds).filter((id) => id !== driverId)
      );
  ...
  const removeSprintDriver = useCallback(
    (driverId: string) => {
      ...
      setSprintTop8((prev) =>
        cleanPickList(prev, sprintTopN, validDriverIds).filter((id) => id !== driverId)
      );
  ...
  const handleSave = useCallback(async () => {
    if (!nextRace) {
      Alert.alert('Cannot Save', 'No upcoming race is available.');
      return;
    }
  ...
  if (!nextRace) {
    return (
      <View style={styles.emptyContainer}>
        <Trophy color={seriesColors.primary} size={34} />
        <Text style={styles.emptyTitle}>No Upcoming Races</Text>
        <Text style={styles.emptyText}>
          Check back when the next race is announced.
        </Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: seriesColors.background }]}>
      ...
      >
        ...
        {activeTab === 'race' && (
          <>
                ...
                {Array.from({ length: raceTopN }).map((_, index) => {
                  ...
                  return (
                    <GridSlot
                      ...
                      onPress={() => openPicker({ type: 'grid', slotIndex: index })}
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.sectionSubtitle}>
                Fastest lap +{flBonus} · {seriesLabels.dnfLabel} +{dnfBonus} · DNS does not count
              </Text>
        ...
        {activeTab === 'sprint' && nextRace.hasSprint && (
          <>
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <View style={styles.sectionHeaderText}>
            ...
            {!sprintLocked && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Available Sprint {seriesLabels.competitors}</Text>
                <Text style={styles.sectionSubtitle}>
      ...
      {(!locked || (nextRace.hasSprint && !sprintLocked)) && (
        <View style={styles.saveBar}>
          <View style={styles.saveBarInfo}>
            ...
            <Text style={styles.saveBarPoints}>{totalPotentialPoints} pts</Text>
          </View>
                  ...
                  >
                    ...
                    <View style={styles.flex}>
                      ...
                      <Text style={styles.pickerTeam}>
                        ...
                        {alreadyPicked && !isCurrent ? ' · already picked' : ''}
                      </Text>
                    </View>
    ...
    <View style={styles.slotFilled}>
      ...
      >
        ...
        <View style={styles.slotMetaRow}>
          <Text style={styles.slotTeamName}>{teamName || driver.shortName}</Text>
