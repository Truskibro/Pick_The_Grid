...
alter table public.user_predictions add column if not exists series_id text default 'f1';
update public.user_predictions set series_id = 'f1' where series_id is null;
alter table public.user_predictions alter column series_id set default 'f1';

-- Add index for series filtering on predictions
...
update leagues set series_id = 'f1' where series_id is null;
